...
async function callRewriteApi(text, fullBody) {
...
function rewriteInAI(event) {
  // Fetch the entire email body (history) first, then get the selection
  Office.context.mailbox.item.body.getAsync(Office.CoercionType.Text, async bodyResult => {
      const fullBody = bodyResult.status === Office.AsyncResultStatus.Succeeded ? (bodyResult.value || "") : "";
    ...
    Office.context.mailbox.item.getSelectedDataAsync(Office.CoercionType.Text, async sel => {
      ...
      try {
        ...
        const data = await callRewriteApi(selected, fullBody);
